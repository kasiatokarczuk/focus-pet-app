  ...
  return (
    <section className={`pet-card ${compact ? 'pet-card--compact' : ''}`.trim()}>
      ...
      <div className="pet-card__stats">
        <StatBar label="XP" value={pet.xp} max={pet.nextLevelXp} tone="mint" />
        <StatBar label="HP" value={pet.hp} max={100} tone="danger" />
        <StatBar label="Energy" value={pet.energy} max={100} tone="warning" />
      </div>
    </section>
  );
}
